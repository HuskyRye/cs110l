...
fn main() {
    ...
    let lines1 = read_file_lines(filename1).expect(&format!("Invalid filename: {filename1}"));
    let lines2 = read_file_lines(filename2).expect(&format!("Invalid filename: {filename2}"));
